...
class TestUpdatePostImpl:
    ...
    async def test_update_post_success(
        self, mock_session, mock_post_model, mock_user, mock_category, update_data
    ):
        # prepare
        with (
            patch.object(
                UpdatePostImpl,
                "_find_post_by_id",
                AsyncMock(return_value=mock_post_model),
            ),
            ...
            result = await repository.update(
                mock_post_model.id, update_data, mock_user.uuid
            )
    ...
    async def test_update_post_not_found(self, mock_session, mock_user, update_data):
        ...
        with patch.object(
            UpdatePostImpl,
            "_find_post_by_id",
            AsyncMock(side_effect=PostNotFoundException()),
        ):
            repository = UpdatePostImpl(mock_session)
    ...
    @pytest.mark.asyncio
    async def test_update_post_user_not_found(
        self, mock_session, mock_post_model, update_data
    ):
        # prepare
        with (
            patch.object(
            ...
            patch.object(
                UpdatePostImpl,
                "_check_author_access",
                AsyncMock(side_effect=UserNotFoundException()),
            ),
        ):
            repository = UpdatePostImpl(mock_session)
            user_id = uuid4()
    ...
    @pytest.mark.asyncio
    async def test_update_post_user_no_access(
        self, mock_session, mock_post_model, mock_user, update_data
    ):
        # prepare
        with (
            patch.object(
            ...
            patch.object(
                UpdatePostImpl,
                "_check_author_access",
                AsyncMock(side_effect=UserHasNoAccessException()),
            ),
        ):
            repository = UpdatePostImpl(mock_session)
    ...
    @pytest.mark.asyncio
    async def test_update_post_category_not_exists(
        self, mock_session, mock_post_model, mock_user, update_data
    ):
        # prepare
        with (
    ...
    async def test_validate_category_exists_success(self, mock_session, mock_category):
        ...
        mock_session.execute.return_value = MagicMock(
            scalar_one=Mock(return_value=mock_category)
        )
        repository = UpdatePostImpl(mock_session)
    ...
    async def test_validate_category_exists_failure(self, mock_session):
        ...
        # action
        with pytest.raises(CategoryDoesNotExistsException):
            await repository._validate_category_exists(999)
