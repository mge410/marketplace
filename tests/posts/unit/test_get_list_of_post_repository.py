...
from src.posts.repositories.implementation.get_list_of_posts_impl import GetListOfPostsImpl
...
class TestGetListOfPostsImpl:
    @pytest.mark.asyncio
    async def test_get_list_of_posts_success(self, mock_session, mock_posts, post_query_params):
        mock_result = MagicMock()
        mock_result.scalars.return_value.unique.return_value.all.return_value = mock_posts
        mock_session.execute.return_value = mock_result

        with unittest.mock.patch.object(GetListOfPostsImpl, 'filter') as mock_filter, \
    ...
    @pytest.mark.asyncio
    async def test_initial_query_includes_selectinload(self, mock_session, post_query_params):
        repository = GetListOfPostsImpl(mock_session)
